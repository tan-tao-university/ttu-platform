...
1. **Do not build ahead of wiring order**:
   ...
   - Do not build Site Settings endpoints against `site_settings` speculatively; design doc 05 §12.2 names four example keys but gives no field-level shape for any of them — wire it once a real Settings Registry (valid keys/types/validation/defaults/scopes) exists.
