...
`tan-tao-university` is on the GitHub **Free** org plan and `ttu-platform` is a **private**
repo. GitHub only offers branch protection rules / rulesets on private repos for organizations
on the **Team** or **Enterprise** plan — confirmed against the live repo:
...
Rulesets cannot be created until the org either upgrades to GitHub Team (or above) or the repo
is made public. Nothing below can be enforced until then; this doc records the exact steps so
whoever has org billing access can apply them the moment the plan changes.
...
Two rulesets, no bypass actors (so admins/owners are not exempt). Requires repo admin access
and a plan that supports private-repo rulesets.
...
Rulesets support an explicit, empty `bypass_actors` list, which is what makes "not even
admins" enforceable — legacy branch protection's "Include administrators" toggle is weaker and
scoped per-branch rule instead of per-actor. Both features are gated by the same plan
requirement on private repos.
