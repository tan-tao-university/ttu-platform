...
`apps/api` has a database connection, a Keycloak auth guard (`src/access/`), and backend domains for Content/Taxonomy, Media/MinIO, Navigation, Redirects, Settings, Audit, and — as of the Component Registry package (`packages/cms-registry`) — a Page/Section/Publish/Rollback CMS domain (`src/cms/`). See root [README.md](README.md) Status, [docs/setup.md](docs/setup.md), and [apps/api/IMPLEMENTATION_STATUS.md](apps/api/IMPLEMENTATION_STATUS.md) for the current, authoritative state.
