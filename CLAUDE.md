...
2. **Documentation & Continuous Updates**: update `docs/` alongside any code change; if a feature exists in code but is missing from `docs/`, add the matching document to `docs/` and render any diagrams (`bun run docs:render-diagrams` per `.agents/skills/diagram-design`); never duplicate raw mermaid code in `.md` when the diagram image is embedded; never hard-wrap markdown prose (one continuous line per paragraph/bullet); committed files are English-only except Vietnamese-first product copy in `apps/web`/`apps/admin`.
...
5. **Major change → update the tracker + changelog, same PR.** A new domain/module, a schema change, a new app surface, or a breaking API change updates the master [IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md) and the corresponding app tracker, and adds an entry to [CHANGELOG.md](CHANGELOG.md). Routine fixes/refactors/doc typos don't need either.
6. **Test Location**: All spec tests for `apps/api` must be placed inside `apps/api/test/`, mirroring `src/` (never colocate `*.spec.ts` in `src/`).
