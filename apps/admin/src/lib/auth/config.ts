 ...
 *
 * Lazily computed and cached on first call, not at module scope: `next build`'s page-data
 * collection step imports every route's module graph — including dynamic (`ƒ`) routes that are
 * never actually executed at build time — so a top-level `requireEnv()` would fail the production
 * build itself over secrets that are only ever needed at container runtime (this app ships as a
 * `next build` Docker image with env injected when the container starts, not when the image is
 * built).
 */
...
export function getAuthConfig(): AuthConfig {
  ...
  /**
   * E.g. `http://localhost:3011` in dev — the fixed, allowlisted origin this app is served from
   * (doc 07 §4: "Redirect URI được allowlist theo môi trường"). Deliberately not derived from the
  ...
  cached = {
     ...
     * directly into the AES-256-GCM key that encrypts the session cookie (`session.ts`).
     */
